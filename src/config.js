...
function rewriteKey(object, oldKey, newKey) {
	const canRename =
		Object.hasOwn(object, oldKey) && !Object.hasOwn(object, newKey);
...
function getLegacyConfig(hugoConfig) {
	...
	const paramsKey = Object.keys(hugoConfig).find(
		(key) => key.toLowerCase() === "params",
	);
	...
	return {
		...
		_array_structures:
			hugoConfig._array_structures ??
			params._array_structures ??
			params._arrayStructures,
		_select_data:
			hugoConfig._select_data ?? params._select_data ?? params._selectData,
	};
}
	...
	const config = {
		...
		multilingual: {
			...
			defaultContentLanguageInSubdir:
				hugoConfig.defaultContentLanguageInSubdir || false,
